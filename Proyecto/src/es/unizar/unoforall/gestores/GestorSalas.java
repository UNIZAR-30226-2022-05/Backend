...
public class GestorSalas {
	private static ConcurrentHashMap<UUID,Sala> salas;
	private static ConcurrentHashMap<UUID,Timer> timersSalas;
	
	static {
		salas = new ConcurrentHashMap<>();
		timersSalas = new ConcurrentHashMap<>();
	}
	...
	public static Map<UUID,Sala> buscarSalas(ConfigSala configuracion) { 
                if (configuracion == null) {
                        return Collections.EMPTY_MAP;
                } else {
                        Map<UUID,Sala> result = new HashMap<>();
                        salas.entrySet().forEach((entry) -> {
	...
	public static void eliminarParticipanteSalas(UUID usuarioID) {
            new HashMap<>(salas).entrySet().forEach((entry) -> {
                UUID salaID = entry.getKey();
                Sala sala = entry.getValue();
